...
let max_int48 = max_int/256
            
let combine (tot1,stat1) (tot2,stat2) =
  if (float tot1*.float tot2) > float max_int32
  then
    (max_int32,[])
  else
    begin
    ...
    compute_mul s1 s2 return a list of (a,b) indicating that there are
     b times where an element can be multiplied a times *)
      ...
      let rec combine_common_col s1 s2  =
        ...
        (res,min nbDefRes new_max,min new_max (nbPerDef1*nbPerDef2),min new_max totalDefRes)
      in
      ...
      let combine_specific (s1:'a stat) (s2:'a stat) (c1:'a list) (c2:'a list) mul =
        ...
        | cols1_specific ->
           List.map (fun c ->
               ...
               c,(res,min new_max (mult mul nbDef),min new_max (mult mul nbPerDef),min new_max (mult mul totalDef))
             ) cols1_specific
      in
      debug (fun () ->
