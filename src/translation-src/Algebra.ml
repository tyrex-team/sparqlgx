      ...
      Not_found ->        
        ...
        let code,keys,cols = match l with 
          ...
          | LeftJoin(a,b) ->
             ...
             if cols_join = []
             then
               "val "^res^"="^code_a^mapkeys cols_a keys_a []^".cartesian("^code_b^mapkeys cols_b keys_b []^").map{case (("^join keys_a cols_a^"),("^join keys_b cols_b^")) => ("^join [] cols_union_some^")}",[],cols_union_some
             else
          ...
          | Join(a,b) ->
             ...
             if cols_join = []
              then
               "val "^res^"="^code_a^mapkeys cols_a keys_a []^".cartesian("^code_b^mapkeys cols_b keys_b []^").map{case (("^join keys_a cols_a^"),("^join keys_b cols_b^")) => ("^join [] cols_union^")}",[],cols_union
              else
